...
import {SalesforceService} from '../util/salesforce.service.promise';
...
export class ClockComponent implements OnInit {
  ...
  ngOnInit() {
    this.sfService
      .remoteAction("AnalogClock.getUser", null).then(res =>
      console.log(res)
    ).catch(err => console.error(err));

    this.sfService
      .remoteAction("AnalogClock.getAccount", null).then(res =>
        this.accounts =res
      /* console.log(res)*/
    ).catch(err => console.error(err));

    this.sfService
      .remoteAction("AnalogClock.deleteAccount", JSON.stringify({id: '0012v00002J4dNA'}))
      .then(res => console.log(res))
      .catch(err => console.error(err));
    this.sfService
      .remoteAction("AnalogClock.addAccount", JSON.stringify({name:'Abhi'}))
      .then(res => console.log(res))
      .catch(err => console.error(err));
    this.sfService
      .remoteAction("AnalogClock.updateAccount", JSON.stringify({id: '0012v00002J4hsO',name:'Rajababu'}))
      .then(res => console.log(res))
      .catch(err => console.error(err));
